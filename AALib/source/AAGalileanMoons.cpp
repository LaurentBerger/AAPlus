...
CAAGalileanMoonsDetails CAAGalileanMoons::CalculateHelper(double JD, double sunlongrad, double betarad, double R, bool bHighPrecision) noexcept
{
  ...
  const double Sigma1 = 0.47259*sin(2*(l1rad - l2rad)) +
                       -0.03478*sin(pi3 - pi4) +
  ...
  const double Sigma2 = 1.06476*sin(2*(l2rad - l3rad)) +
...
void CAAGalileanMoons::Rotations(double X, double Y, double Z, double I, double psi, double i, double omega, double lambda0, double beta0, double& A6, double& B6, double& C6) noexcept
{
  ...
  const double A1 = X;
  const double B1 = Y*cos(I) - Z*sin(I);
  const double C1 = Y*sin(I) + Z*cos(I);

  //Rotation towards the ascending node of the orbit of jupiter
  const double A2 = A1*cos(phi) - B1*sin(phi);
  const double B2 = A1*sin(phi) + B1*cos(phi);
  const double C2 = C1;

  //Rotation towards the plane of the ecliptic
  const double A3 = A2;
  const double B3 = B2*cos(i) - C2*sin(i);
  const double C3 = B2*sin(i) + C2*cos(i);

  //Rotation towards the vernal equinox
  const double A4 = A3*cos(omega) - B3*sin(omega);
  const double B4 = A3*sin(omega) + B3*cos(omega);
  const double C4 = C3;

  const double A5 = A4*sin(lambda0) - B4*cos(lambda0);
  const double B5 = A4*cos(lambda0) + B4*sin(lambda0);
  const double C5 = C4;
